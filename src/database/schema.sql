...
CREATE TABLE IF NOT EXISTS dim_protein (
    protein_key     INT             NOT NULL   COMMENT 'ETL-assigned surrogate key',
...
CREATE TABLE IF NOT EXISTS dim_drug (
    drug_key        INT             NOT NULL   COMMENT 'ETL-assigned surrogate key',
...
CREATE TABLE IF NOT EXISTS dim_article (
    article_key     INT             NOT NULL   COMMENT 'ETL-assigned surrogate key',
...
-- Linked to DimProtein via the business key (uniprot_id) rather than
-- protein_key because a single protein maps to many structures.
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS dim_structure (
    structure_key   INT             NOT NULL   COMMENT 'ETL-assigned surrogate key',
    ...
    uniprot_id      VARCHAR(20)     NOT NULL   COMMENT 'UniProt accession — soft FK to dim_protein',
    resolution      FLOAT                      COMMENT 'Crystallographic resolution in Angstroms (Å = 1e-10 m)',
    ...
    INDEX   ix_dim_structure_uniprot       (uniprot_id),

    CONSTRAINT fk_structure_protein
        FOREIGN KEY (uniprot_id)
        REFERENCES  dim_protein (uniprot_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE
)
ENGINE = InnoDB
...
-- -----------------------------------------------------------------------------
-- FactBioactivity
-- Grain: one experimental measurement (e.g. IC50) per
--        (drug, protein, publication) triple.
-- Measures: standard_value, pchembl_value, confidence_score
-- Degenerate dimensions: standard_type, standard_units, assay_type,
--                        assay_description
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS fact_bioactivity (
    ...
    pchembl_value       FLOAT                      COMMENT '−log₁₀(molar potency) — comparable across assays',
